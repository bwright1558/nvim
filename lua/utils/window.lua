...
--- window.focus("h") -- move/create left
---
--- Optionally map keys (example):
--- vim.keymap.set("n", "<C-h>", function() require("utils.window").focus("h") end)
--- vim.keymap.set("n", "<C-j>", function() require("utils.window").focus("j") end)
--- vim.keymap.set("n", "<C-k>", function() require("utils.window").focus("k") end)
--- vim.keymap.set("n", "<C-l>", function() require("utils.window").focus("l") end)
...
function M.focus(dir)
  if not split_cmd[dir] then
    vim.notify("window.focus: dir must be h/j/k/l, got " .. tostring(dir), vim.log.levels.ERROR)
    return
  end
